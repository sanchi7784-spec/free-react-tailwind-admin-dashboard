...
export default function UserDropdown() {
  ...
  // Determine profile link based on user domain
  // Gold/MPay users (domain 0 & 1) -> /profile
  // Ecommerce users (domain 2) -> /ecommerce/reports/sales (or no profile link)
  const profileLink = canAccessGold() && !canAccessEcommerce() 
    ? "/profile"  // MPay user (domain 1)
    : canAccessGold() && canAccessEcommerce()
    ? "/profile"  // Super admin (domain 0) - can use gold profile
    : "/ecommerce/reports/sales";  // Ecom user (domain 2) - redirect to reports
    
  const showProfileLink = isEcommerceAuthenticated();
  return (
    <div className="relative">
      <button
        onClick={toggleDropdown}
        className="flex items-center text-gray-100 dropdown-toggle dark:text-gray-400"
      >
      ...
      <Dropdown isOpen={isOpen} onClose={closeDropdown} className="absolute right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark">
        <div>
          ...
          {userRole && (
            <span className="mt-1 inline-block px-2 py-0.5 text-xs font-medium rounded-full bg-brand-50 text-brand-700 dark:bg-brand-900/30 dark:text-brand-400">
              {userRole}
            </span>
          )}
        </div>
        ...
        <a
          onClick={() => {
            clearAuth();
            clearEcommerceAuth();
            // Clear all session data to prevent conflicts
